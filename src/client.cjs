    ...
    /**
     * The stock conversation shell owns the chip immediately before the
     * workspace slot, so a slot plugin cannot replace that one button. Keep
     * its user-facing copy aligned with this plugin without touching any DSH
     * state or relying on generated CSS class names.
     */
    ...
    function Icon({ name, size = 18 }) {
      ...
      return h("svg", common, ...(paths[name] || paths.folder));
    }
    ...
    function DirectoryBrowserModal({ open, initialPath, listDirectory, onPick, onClose, t }) {
      ...
      const load = React.useCallback(async (path) => {
        ...
        setError("");
        try {
          const next = await listDirectory(path || undefined, controller.signal);
      ...
      return ReactDOM.createPortal(
        h("div", { className: "dshp-backdrop", onMouseDown: (event) => { if (event.target === event.currentTarget) onClose(); } },
          h("section", { className: "dshp-modal dshp-dir-modal", role: "dialog", "aria-modal": true, "aria-label": t("chooseProjectFolder") },
            ...
            h("div", { className: "dshp-dir-crumbs" },
            ...
            h("div", { className: "dshp-dir-list" },
              loading
                ? h("div", { className: "dshp-dir-status" }, t("folderLoading"))
                : error
                  ? h("div", { className: "dshp-dir-status dshp-error", role: "alert" }, error)
                  : entries.length
                    ? entries.map((entry) => h("button", { key: entry.path, type: "button", className: "dshp-dir-row", title: entry.path, onClick: () => load(entry.path) }, h(Icon, { name: "folder", size: 18 }), h("span", null, entry.name)))
                    : h("div", { className: "dshp-dir-status" }, t("folderEmpty"))
            ),
            listing ? h("div", { className: "dshp-dir-path", title: listing.path }, listing.path) : null,
            h("div", { className: "dshp-dir-footer" },
              ...
              h("div", { className: "dshp-modal-actions", style: { marginTop: 0 } },
    ...
    function CreateProjectModal({ open, projects, createWorkspace, renameWorkspace, pickDirectory, listDirectory, onCreated, onClose, t }) {
      ...
      return h(React.Fragment, null,
        ReactDOM.createPortal(
          h("div", { className: "dshp-backdrop", onMouseDown: (event) => { if (event.target === event.currentTarget) close(); } },
            h("section", { className: "dshp-modal", role: "dialog", "aria-modal": true, "aria-label": t("createProject") },
            h("div", { className: "dshp-modal-head" },
            ...
            h("div", { className: "dshp-name-field" },
              ...
              h("input", {
                ...
                onKeyDown: (event) => { if (event.key === "Enter" && path && !busy) create(); }
              })
            ),
            ...
            h("div", { className: "dshp-error", role: error ? "alert" : undefined }, error),
            h("div", { className: "dshp-modal-actions" },
              ...
              h("button", { type: "button", className: "dshp-button dshp-button-primary", disabled: busy || picking || !name.trim() || !path, onClick: create }, busy ? t("creating") : t("createProject"))
            )
            )
          ),
          document.body
        ...
        h(DirectoryBrowserModal, {
          ...
          listDirectory,
          onPick: adoptFolder,
          onClose: () => { setBrowserOpen(false); setPicking(false); },
          t
        })
      );
    }
    ...
    function ProjectPicker({ open, anchorRef, selectedId, onPick, onClose, useWorkspaces, useSessions, createWorkspace, renameWorkspace, pickDirectory, listDirectory, startDefaultSession, isDefaultWorkspace, t }) {
      ...
      const chooseDefault = async () => {
        ...
        setDefaultBusy(true); setDefaultError("");
        try {
      ...
      const menu = open && !modalOpen ? ReactDOM.createPortal(
        h("div", { className: "dshp-menu", style: position, role: "menu" },
          ...
          h("div", { className: "dshp-menu-footer" },
            h("button", { type: "button", className: "dshp-new-item dshp-default-item", disabled: defaultBusy, onClick: chooseDefault },
              h(Icon, { name: "chat", size: 18 }),
              h("span", { className: "dshp-default-copy" },
            ...
            h("button", { type: "button", className: "dshp-new-item", onClick: () => { setModalOpen(true); onClose(); } }, h(Icon, { name: "plus", size: 18 }), h("span", null, t("newProject")))
          )
        ),
        document.body
      ) : null;
      ...
      return h(React.Fragment, null,
        ...
        h(CreateProjectModal, {
          ...
          onCreated: (workspaceId) => { setModalOpen(false); onPick(workspaceId); },
          t
        })
      );
    }
    ...
    function reconcileOrder(ids, stored) {
      ...
      const seen = new Set();
      const ordered = [];
      for (const id of stored || []) if (valid.has(id) && !seen.has(id)) { ordered.push(id); seen.add(id); }
    ...
    function movedAt(ids, movingId, targetId, after) {
      ...
      if (index < 0) next.push(movingId); else next.splice(index + (after ? 1 : 0), 0, movingId);
      return next;
    }
    ...
    function EditProjectModal({ project, projects, renameWorkspace, onClose, t }) {
      ...
      const save = async () => {
        ...
        try { await renameWorkspace(project.workspaceId, trimmed); onClose(); }
        catch (reason) { setError(format(t, "actionFailed", { message: reason instanceof Error ? reason.message : String(reason) })); }
        finally { setBusy(false); }
      };
      return ReactDOM.createPortal(
        h("div", { className: "dshp-backdrop", onMouseDown: (event) => { if (event.target === event.currentTarget && !busy) onClose(); } },
          h("section", { className: "dshp-modal dshp-small-modal", role: "dialog", "aria-modal": true },
            h("div", { className: "dshp-modal-head" },
            ...
            h("div", { className: "dshp-name-field" },
              ...
              h("input", { ref: inputRef, value, maxLength: 80, onChange: (event) => { setValue(event.target.value); setError(""); }, onKeyDown: (event) => { if (event.key === "Enter") save(); } })
            ),
    ...
    function ConfirmModal({ request, busy, error, onConfirm, onClose, t }) {
      ...
      return ReactDOM.createPortal(
        h("div", { className: "dshp-backdrop", onMouseDown: (event) => { if (event.target === event.currentTarget && !busy) onClose(); } },
          h("section", { className: "dshp-modal dshp-small-modal", role: "alertdialog", "aria-modal": true },
            h("div", { className: "dshp-modal-head" },
    ...
    function SessionRow({ session, project, current, pinned, draggable, dragOver, onOpen, onTogglePin, onArchive, onDragStart, onDragOver, onDrop, onDragEnd, t }) {
      ...
      return h("div", {
        ...
        h("div", { className: "dshp-session-actions" },
          h("button", { type: "button", className: "dshp-mini-button", "data-active": pinned ? "true" : "false", title: pinned ? t("unpinChat") : t("pinChat"), onClick: (event) => { event.stopPropagation(); hover.leave(); onTogglePin(session.id); } }, h(Icon, { name: "pin", size: 15 })),
          h("button", { type: "button", className: "dshp-mini-button", title: t("archiveChat"), onClick: (event) => { event.stopPropagation(); hover.leave(); onArchive(session.id); } }, h(Icon, { name: "archive", size: 15 }))
        ),
        h(PreviewPortal, { position: hover.position },
    ...
    function ProjectRow({ project, sessions, pinned, favorite, menuOpen, draggable, dragOver, onStart, onMenu, onDragStart, onDragOver, onDrop, onDragEnd, t }) {
      ...
      return h("div", {
        ...
        h("div", { className: "dshp-project-actions" },
          ...
          h("button", { type: "button", className: "dshp-mini-button", title: t("newSession"), onClick: (event) => { event.stopPropagation(); hover.leave(); onStart(project.workspaceId); } }, h(Icon, { name: "compose", size: 16 }))
        ),
    ...
    function ArchiveCenterModal({ open, archivedIds, projects, sessionState, restoreSession, onClose, t }) {
      ...
      React.useEffect(() => {
        if (!open) return;
        setError("");
        const onKey = (event) => { if (event.key === "Escape" && !busyId) onClose(); };
        window.addEventListener("keydown", onKey);
      ...
      const restore = async (id) => {
        ...
        try { await restoreSession(id); }
        catch (reason) { setError(format(t, "restoreFailed", { message: reason instanceof Error ? reason.message : String(reason) })); }
        finally { setBusyId(""); }
      };
      return ReactDOM.createPortal(
        h("div", { className: "dshp-backdrop", onMouseDown: (event) => { if (event.target === event.currentTarget && !busyId) onClose(); } },
          h("section", { className: "dshp-modal dshp-archive-modal", role: "dialog", "aria-modal": true, "aria-label": t("archiveCenter") },
            h("div", { className: "dshp-modal-head" },
            ...
            h("div", { className: "dshp-archive-list" },
              rows.length ? rows.map(({ id, session, project }) => h("div", { key: id, className: "dshp-archive-row" },
                h("span", { className: "dshp-search-result-icon" }, h(Icon, { name: "archive", size: 18 })),
                h("div", { className: "dshp-archive-copy" },
    ...
    function SidebarProjects({ wide, expandSidebar, useWorkspaces, useSessions, openSession, startSession, startDefaultSession, isDefaultWorkspace, createWorkspace, renameWorkspace, pickDirectory, listDirectory, deleteWorkspace, insertWorkspaceBefore, archiveSession, restoreSession, insertSessionBefore, openPath, searchSessions, searchResultLimit, t }) {
      ...
      return h(React.Fragment, null,
        h("div", { className: "dshp-sidebar", "data-dragging": drag ? "true" : "false" },
          h("div", { className: "dshp-sidebar-scroll" },
            h("div", { className: "dshp-section-head" },
              h("div", { className: "dshp-section-title" }, prefs.groupBy === "project" ? t("projects") : t("chats")),
              h("div", { className: "dshp-section-actions" },
                h("button", { type: "button", className: "dshp-mini-button", "data-active": searchOpen ? "true" : "false", title: t("globalSearch"), onClick: () => { setSearchOpen((value) => !value); setMenu(null); } }, h(Icon, { name: "search", size: 16 })),
                h("button", { type: "button", className: "dshp-mini-button", "data-active": archiveOpen ? "true" : "false", title: t("archiveCenter"), onClick: () => { setArchiveOpen(true); setMenu(null); } }, h(Icon, { name: "archive", size: 16 })),
            ...
            searchOpen ? h(React.Fragment, null,
              h("div", { className: "dshp-sidebar-search" },
                ...
                h("input", { ref: searchInputRef, value: searchQuery, placeholder: t("searchEverything"), "aria-label": t("globalSearch"), onChange: (event) => setSearchQuery(event.target.value), onKeyDown: (event) => { if (event.key === "Escape") closeSearch(); } }),
              ...
              ? (flatSessions.length ? flatSessions.map((session) => renderSessionRow(session, projectForSession(session.id), true)) : h("div", { className: "dshp-sidebar-empty" }, t("noChats")))
              : (projects.length
                  ? orderedProjects.map((project) => {
                      ...
                      return h("div", { key: project.workspaceId, className: "dshp-project-block" },
                        h(ProjectRow, {
                          project,
                          sessions,
                        ...
                        sessions.length > 4 ? h("button", { type: "button", className: "dshp-more", onClick: () => setExpanded((current) => { const next = new Set(current); if (next.has(project.workspaceId)) next.delete(project.workspaceId); else next.add(project.workspaceId); return next; }) }, showAll ? t("showLess") : t("showMore")) : null
                      );
                    })
                  : h("div", { className: "dshp-sidebar-empty" }, t("noProjects"))),
            prefs.groupBy === "project" && stray.length ? h(React.Fragment, null,
        ...
        h(CreateProjectModal, {
          ...
          onCreated: (workspaceId) => { setCreateOpen(false); startSession(workspaceId); },
          t
        }),
    ...
    function apply(ctx) {
      ...
      const injected = () => ({
        ...
        restoreSession: typeof ctx.workspaces.unarchiveSession === "function"
          ? (sessionId) => ctx.workspaces.unarchiveSession(sessionId)
          : null,
