...
| 侧边栏 | 按项目或单列表显示，提供“最近”、置顶、收藏和拖动排序 |
| 项目管理 | 重命名、在文件管理器中打开、归档聊天、移除项目注册 |
| 全局搜索 | 搜索项目名、聊天标题；启用 DSH 索引后还能搜索聊天正文 |
...
插件不建立第二套项目数据库，也不会在卸载时删除项目文件夹或会话记录。按项目/单列表视图、排序方式、置顶、收藏等显示偏好保存在浏览器 `localStorage`；清除站点数据只会重置这些偏好。
